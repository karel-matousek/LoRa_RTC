...
static void OnRxDone(uint8_t *payload, uint16_t size, int16_t rssi, int8_t LoraSnr_FskCfo)
{
  /* USER CODE BEGIN OnRxDone */
	...
	if (crc_bytes[0] == time_crc[0] && crc_bytes[1] == time_crc[1]) {
		...
		time_unformatted += 7200; // + 2 hour (Summer Czech Republic time)
		// timeUnformatted -= 18;	 // - 18 seconds

		uint32_t ticks_from_last_reset = __HAL_TIM_GET_COUNTER(&htim2);

		if (timer_running_flag == 1) {
			uint32_t ticks_in_period = (uint32_t)(htim2.Init.Period);
			uint16_t secs_between_beacons = (uint16_t)(time_unformatted - prev_received_time);

			int64_t total_ticks = (int64_t)ticks_in_period * (int64_t)timer_periods + (int64_t)ticks_from_last_reset;
			int64_t correct_ticks = (int64_t)ticks_in_period * (int64_t)secs_between_beacons;
			int64_t diff = total_ticks - correct_ticks;
			error_ms = (int16_t)(diff / 48000); // error in ms

			htim2.Init.Period = (uint32_t)((int32_t)(htim2.Init.Period) + (int32_t)(diff / secs_between_beacons));
		}
...
//		uint8_t secs;
		...
		// format_time(time_unformatted, &hours, &mins, &secs);
		...
		printf("\r\nTime: %02u:%02u:%02u\r\n", td.hours, td.minutes, td.seconds);
		printf("Error: %d ms       \r\n", error_ms);
		printf("Error bin: %d%d%d%d%d%d%d%d%d%d%d%d%d%d%d%d\r\n",
		...
		printf("Current timer period: %lu\r\n", htim2.Init.Period);
