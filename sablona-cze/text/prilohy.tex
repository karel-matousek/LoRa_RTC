...
Pro sazbu algoritmů/pseudokódu je možné využít balíček \texttt{algorithm2e} nabízející bohaté možnosti sazby.
Na algoritmy je možno se klasicky odkazovat křížovými odkazy, jako zde: viz alg.~\ref{alg:PHADQ}.
...
\begin{algorithm}%[h]
	...
	Inicializuj $x^{(0)}, p^{(0)}, q^{(0)}$ a $\omega_\mathbf{s}$
	\\
	\For{$i=0,1,\dots$}{
		$q^{(i+1)}=\textup{clip}_\lambda(q^{(i)}+\sigma DR_{\omega_\mathbf{s}}G_g x)$
		\;
		$u = p^{(i)}-\tau G^*_g R^*_{\omega_\mathbf{s}}D^*q^{(i+1)}$ \quad\%\,pomocné
		\;
		$p^{(i+1)} = \frac{1}{\tau+1}(\tau \ \textup{proj}_\Gamma(u)+u)$
		\;
